...
# Blocks git commit if any testable source file in src/ is missing
...
#         constants, mocks, test utils, main.tsx, bootstrap.tsx,
#         appInsights.ts, global.d.ts
...
# Skip if src doesn't exist (e.g. running from wrong directory)
if [ ! -d "$WEB_SRC" ]; then
  exit 0
fi
...
while IFS= read -r src_file; do
  ...
  # Skip bootstrap.tsx (MF async boundary — entry-equivalent)
  [ "$basename" = "bootstrap.tsx" ] && continue
  ...
  # Skip scaffold placeholders. The /build-architecture → /build-scaffold →
  # /build-application workflow ships scaffolded modules without tests so
  # later slices can fill them in. Each placeholder file declares itself in
  # its own header docstring with "lands in slice <N>" — when the slice
  # implementation arrives, that marker is removed (along with the placeholder
  # body) and the test ships at the same time. Files that retain the marker
  # are by-design test-free at this commit.
  if head -25 "$src_file" 2>/dev/null | grep -qE 'lands in slice [0-9]'; then
    continue
  fi
