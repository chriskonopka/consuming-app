...
| [`chat.ts`](../../shared/types/chat.ts)                 | `ChatSession`, `StreamingState`, `LocalMessage`, `SimulatedPhase`, `toLocalMessage()`. (Model picker types removed — v1 hardcodes `llmProvider: 'Claude'` per REQUIREMENTS.md §4.9.)                                                                                                     | `features/chat/`                                  |
| [`citation.ts`](../../shared/types/citation.ts)         | `Citation` (alias of API's `CitationData`), `AuditedCitation`, `CitationAuditStatus`, `SourceDocumentGroup`, `auditCitation()`, `groupCitationsByDocument()`                                                                                                                             | `features/citations/`                             |
