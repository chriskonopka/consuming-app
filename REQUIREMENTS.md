...
- v1 DOES apply the **drift guard** (§5.6) — a rendered highlight covering > 50% of visible page height is rejected as "Couldn't locate this quote on the page." This catches the case where API coordinates land on a region that doesn't make visual sense. (Threshold raised from 25% on 2026-05-13 to fit the API's section-sized citation rectangles; tightening back down depends on server-side citation pipeline returning snippet-tight bounds.)
...
3. **Drift guard** — if the rectangle spans more than 50% of the visible page height, reject the highlight and show "Couldn't locate this quote on the page." (Catches occasional bad coordinates. Raised from 25% on 2026-05-13.)
...
### Citations & viewer
- A citation whose API-supplied rectangle covers > 50% of page height renders with strike-through and "Couldn't locate" tooltip — never as a misleading mark.
